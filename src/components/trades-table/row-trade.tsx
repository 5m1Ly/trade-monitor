...
export const TradeRow = ({
  ...
  const symbol =
    trade.currency === "EUR" ? "€" : trade.currency === "USD" ? "$" : "~";
  ...
  if (trade.active) {
    const currentValue =
      trade.currency === "EUR" ? currentValueEUR : currentValueUSD;
    const profit = trade.currency === "EUR" ? profitEUR : profitUSD;

    return (
      <Row>
        {/* date */}
        <Cell>{trade.buy.date}</Cell>

        {/* price */}
        <Cell>
          {trade.buy.price.toLocaleString("en-US", {
            useGrouping: false,
        ...
        {/* amount */}
        <Cell pos="right">{format(trade.amount)}</Cell>
        ...
        {/* actions */}
        <TableCell className="text-right p-0 pr-0.5">
          <Button size="sm" variant="secondary" onClick={onSellClick}>
            MaS
          </Button>
        </TableCell>
      </Row>
  ...
  return (
    <Row>
      {/* date */}
      <Cell>{trade.sell.date}</Cell>

      {/* price */}
      <Cell>
        {trade.sell.price.toLocaleString("en-US", {
          useGrouping: false,
          minimumFractionDigits: 8,
          maximumFractionDigits: 8,
        })}{" "}
