...
export default function TradesTable({
  ...
  const rows = shibaInuTrades
    ...
    .map((t) => {
      ...
      if (t.currency === "EUR") {
        ...
        expenseUSD =
          rates[t.tokenId] && rates[t.tokenId].usd
            ? expenseEUR * rates[t.tokenId].usd!
            : null;
      } else {
        expenseUSD = t.amount * t.buy.price;
        expenseEUR =
          rates[t.tokenId] && rates[t.tokenId].eur
            ? expenseUSD * rates[t.tokenId].eur!
            : null;
      }
      ...
      if (!t.active && t.sell.price != null) {
        ...
        if (t.currency === "EUR") {
          ...
          currentValueUSD =
            rates[t.tokenId] && rates[t.tokenId].usd
              ? currentValueEUR * rates[t.tokenId].usd!
              : null;
        } else {
          currentValueUSD = t.amount * t.sell.price;
          currentValueEUR =
            rates[t.tokenId] && rates[t.tokenId].eur
              ? currentValueUSD * rates[t.tokenId].eur!
              : null;
        }
        profitEUR =
  ...
  const activeProfitableRows = rows.filter((r) => {
  ...
  const activeLossingRows = rows.filter((r) => {
    if (!r.trade.active) return false;
    // Use EUR profit as primary, fall back to USD
    const profit = r.profitEUR ?? r.profitUSD;
    return profit != null && profit <= 0;
  ...
  const openSubtotal = calculateSubtotal([
    ...activeProfitableRows,
    ...activeLossingRows,
  ]);
  ...
  const totals = {
    ...
    currentValueEUR: activeRows.reduce(
      (sum, r) => sum + (r.currentValueEUR ?? 0),
      0,
    ),
    currentValueUSD: activeRows.reduce(
      (sum, r) => sum + (r.currentValueUSD ?? 0),
      0,
    ),
    // Total profit = active unrealized profit + closed realized profit
    profitEUR:
      activeRows.reduce((sum, r) => sum + (r.profitEUR ?? 0), 0) +
      closedSubtotal.profitEUR,
    profitUSD:
      activeRows.reduce((sum, r) => sum + (r.profitUSD ?? 0), 0) +
      closedSubtotal.profitUSD,
  };
            ...
            {activeProfitableRows.length > 0 && (
              <TotalsRow
                ...
                delta={{
                  ...
                  nominal: profitSubtotal.profitEUR
                    ? profitSubtotal.profitEUR
                    : profitSubtotal.profitUSD
                      ? profitSubtotal.profitUSD
                      : 0,
                }}
              />
            ...
            {activeLossingRows.length > 0 && (
              <TotalsRow
                ...
                delta={{
                  ...
                  nominal: lossSubtotal.profitEUR
                    ? lossSubtotal.profitEUR
                    : lossSubtotal.profitUSD
                      ? lossSubtotal.profitUSD
                      : 0,
                }}
              />
            ...
            {(activeProfitableRows.length > 0 ||
              ...
              <TotalsRow
                ...
                delta={{
                  ...
                  nominal: openSubtotal.profitEUR
                    ? openSubtotal.profitEUR
                    : openSubtotal.profitUSD
                      ? openSubtotal.profitUSD
                      : 0,
                }}
              />
            ...
            {inactiveRows.length > 0 && (
              <TotalsRow
                ...
                delta={{
                  ...
                  nominal: closedSubtotal.profitEUR
                    ? closedSubtotal.profitEUR
                    : closedSubtotal.profitUSD
                      ? closedSubtotal.profitUSD
                      : 0,
